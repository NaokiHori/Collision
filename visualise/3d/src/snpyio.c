...
#define SNPYIO_MESSAGE(level, ...){      \
  fprintf(stderr, "[NPYIO %s]\n", level);      \
...
/* logger */
#if defined(SNPYIO_DO_LOGGING)
#define SNPYIO_LOGGING(...){          \
  SNPYIO_MESSAGE("LOG", __VA_ARGS__); \
}
#else
#define SNPYIO_LOGGING(...)
#endif
...
/* dump error message */
#define SNPYIO_ERROR(...){              \
  SNPYIO_MESSAGE("ERROR", __VA_ARGS__); \
}

/* fatal error, dump error message and abort */
#define SNPYIO_FATAL(...){              \
  SNPYIO_MESSAGE("FATAL", __VA_ARGS__); \
  exit(EXIT_FAILURE);                         \
}
